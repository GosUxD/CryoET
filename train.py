...
from utils.utils import merge_points_by_confidence, BNUpdateWrapper
...
parser.add_argument("-C", "--config", help='Config File', type=str, default='cfg')
...
for fold in range(cfg.n_folds):
    ...
    for epoch in range(cfg.epochs):
        ...
        if epoch + 1 == cfg.epochs:
            update_bn(BNUpdateWrapper(train_dataloader), swa_model, device=cfg.device)
            ...
            if (epoch + 1) % cfg.eval_epochs == 0 or (epoch + 1) == cfg.epochs:
                ...
                for index, data in enumerate(tqdm(val_dataloader, desc=f'Val epoch {epoch}', ascii=' >=')):
                    ...
                    component, confidences = find_connected_component_with_confidence(seg_output[:, 1:], 
                                                              threshold=[0.5,0.25,0.5,0.25,0.5], max_radius=25)
                ...
                if cfg.logging and epoch >= 9:
                    ...
                    #save SWA model
                    torch.save({"model": swa_model.state_dict(), "optimizer": optimizer.state_dict()}, f'logs/checkpoints/{neptune_run._sys_id}/swa_epoch_{epoch}_fold{cfg.fold}.pth')
