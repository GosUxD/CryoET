...
def prepare_tomograms(cfg, mode):
    ...
    if mode == 'train':
        for idx, tomogram_name in enumerate(cfg.train_tomograms):
            tomograms.append(z_score_normalize(np.ascontiguousarray(load_tomogram(tomogram_name, type='denoised'))))
    elif mode == 'val':
        ...
        for idx, tomogram_name in enumerate(cfg.test_tomograms):
            tomogram = z_score_normalize(np.ascontiguousarray(load_tomogram(tomogram_name, type='denoised')))
...
class CryoDataset(Dataset):
    def __init__(self, cfg, mode = 'train'):
        ...
        self.st = SpatialTransform_2(
            ...
            random_crop=True,
            p_el_per_sample=0.2, p_rot_per_sample=0.5, p_scale_per_sample=0.3
        )
    ...
    def __sample(self, point, bound, radius=6):
        # point: z, y, x
        if self.mode == 'train':
            shift_amount = self.shift - radius - self.cfg.discard_range
