...
parser.add_argument("-C", "--config", help='Config File', type=str, default='cfg_mcds_inference')
...
for fold in range(cfg.n_folds):
    ...
    weights = f'logs/checkpoints/CRYOET-308/epoch_{150}_fold{fold}.pth'
    ...
    for index, data in enumerate(tqdm(test_dataloader)):
            ...
            if cfg.mixed_precision:
                with autocast():
                    ...
                    _, seg_flip = model(img_flip)
                    _, seg_flip2 = model(img_flip2)          
            ...
            component, confidences = find_connected_component_with_confidence(seg_output[:, 1:], 
                                                threshold=[0.5, 0.3, 0.5, 0.3, 0.5], max_radius=25)
